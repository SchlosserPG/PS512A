...
### String 
-   In R, a _string_ is a sequence of characters enclosed in quotes, used to represent text data. 
* R accepts single quotes or double quotes when marking a _string_. However, if you use a single quote to start, use a single quote to end. The same for double quotes - ensure the pairing is the same quote type.
...
# This is compared to integer division below
49 %/% 4  #integer division: = 12
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/Hem4uoChTfU?si=y6maq2PDItpJ-ufL" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
* After your working directory is set up, then you can read in datasets into RStudio. 
...
-   We will break down the mutate() function and the piping operator a more in the next module. For now, here is an example of how to recode data into numerical values.
...
-   If we use the read.csv() command, we can sometimes do this by setting an argument $stringsAsFactors=TRUE$. We will do this later in the semester.

### Numerical data type

#### Real Number
* In R, a real number refers to any numerical value that includes both integers and decimals, as opposed to categorical or character data. The as.numeric() function is used to explicitly convert data into numeric format, allowing R to treat it as a real number for calculations. However, if the input to as.numeric() cannot be logically converted (e.g., non-numeric characters), R will return NA with a warning, highlighting the importance of ensuring the input is suitable for numeric conversion. 
-   The as.numeric() function takes an input vector, such as characters or factors, and attempts to coerce it into numeric values. For instance, as.numeric(c("1", "2", "3")) returns a numeric vector of 1, 2, and 3. If applied to factors, it returns the underlying integer codes, not the original levels, so caution is needed to avoid misinterpretation. When conversion is not possible (e.g., as.numeric("abc")), it results in NA with a warning.
