...
Use the following prompts with our chatbot below to explore more about data preparation and visualization with `ggplot2` further.
...
**What comes next:** Module 5 introduces hypothesis testing and t-tests — the first formal inferential tools in the course. The habits built here carry forward directly: a t-test result is only meaningful if the data were cleaned correctly, variable types are right, and missing values were handled deliberately. Every analytical judgment you made in this module — which observations to keep, how to recode categories, whether to impute or drop missing values — shapes the validity of every inferential result that follows.
