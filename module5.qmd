...
Understanding the logic in this module is what gives you the ability to catch these errors. Statistical significance, practical significance, and correct test selection are all human judgment calls — not outputs you can read off a p-value alone.
...
**Equal variances** (for the independent samples t-test) can be checked with the `var.test()` function in base R. If variances are unequal, R's default Welch correction already handles it — just make sure you are not overriding it with `var.equal = TRUE`.
